let imageViewer = {
    ...
    e: {
        ...
        enlargedImageElement: document.getElementById("enlarged_image_display"),
...
imageViewer.evtCallbacks = {
    imageClicked: function(mouseEvent) {
        ...
        let allImages = Array.from(photoContainer.getElementsByClassName('photo_img'));
...
imageViewer.addListeners = function() {
    let photoImages = document.getElementsByClassName('photo_img');

    for (let i = 0; i < photoImages.length; i++) {
        photoImages[i].addEventListener("click", this.evtCallbacks.imageClicked.bind(this));
    }
    ...
    window.addEventListener("click", (e) => {
        if (e.target == this.e.enlargedPhotoContainer) {
